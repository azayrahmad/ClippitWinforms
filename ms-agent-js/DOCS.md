...
**Interruptions:** When `agent.play('NewAnim')` is called while another animation is active (such as a long idle animation):
...
4.  If an `exitBranch` exists, it jumps directly to that frame. This allows the agent to take a "shortcut" to finish its current action gracefully rather than waiting for the entire animation to play through.
5.  Once the exit sequence reaches the neutral frame (Frame 0), the current animation promise resolves.
6.  The new animation then starts immediately.

This system ensures that the agent feels responsive to user input while maintaining visual continuity by not simply "snapping" to the next state.
